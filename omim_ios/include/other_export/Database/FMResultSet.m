...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        if(PRINT_LOG)NSLog(@"Warning: There seem to be no columns in this set.");
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
			...
			if (SQLITE_LOCKED == rc) {
				...
				if (rc != SQLITE_LOCKED) {
					if(PRINT_LOG)NSLog(@"Unexpected result from sqlite3_reset (%d) rs", rc);
				}
			}
            usleep(20);
            ...
            if ([parentDB busyRetryTimeout] && (numberOfRetries++ > [parentDB busyRetryTimeout])) {
                ...
                if(PRINT_LOG)NSLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [parentDB databasePath]);
            ...
            if(PRINT_LOG)NSLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        } 
            ...
            // wtf?
            if(PRINT_LOG)NSLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    if(PRINT_LOG)NSLog(@"Warning: I could not find the column named '%@'.", columnName);
