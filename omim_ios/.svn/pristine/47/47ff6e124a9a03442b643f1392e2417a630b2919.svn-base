...
- (void)loadMoreData{
    ...
    NSInteger count = self.moment_array.count;
    if (count> 0) {
...
-(void)didLoadMoreMoments:(NSNotification*)notif
{
    NSError* error = [[notif userInfo] valueForKey:WT_ERROR];
    if (error.code == NO_ERROR) {
        int download_moment_count = [[[notif userInfo] valueForKey:@"fileName"] intValue];
        int existing_moment_count = (int)self.moment_array.count;
...
-(void)setMoment_array:(NSMutableArray *)moment_array{
    ...
    NSInteger count = _moment_array.count;
