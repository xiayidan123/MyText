...
//            displayLink = nil;
        ...
        [UIView animateWithDuration:0.5 delay:0.0f usingSpringWithDamping:0.4f initialSpringVelocity:0.0f options:UIViewAnimationOptionCurveEaseInOut animations:^{
            if (fabs(self.frontView.center.x - oldBackViewCenter.x) > 22 && fabs(self.frontView.center.y - oldBackViewCenter.y) > 22) {
                self.frontView.hidden = YES;
                [Database setChatMessageAllReaded:self.msg.chatUserName];
            }else{
...
//-(void)setNumberOfBubble:(NSUInteger)numberOfBubble{
//    _numberOfBubble = numberOfBubble;
//    if (_numberOfBubble == 0){
//        self.hidden = YES;
//    }else{
//        self.hidden = NO;
//    }
//     self.bubbleLabel.text = [NSString stringWithFormat:@"%d",_numberOfBubble];
//}
