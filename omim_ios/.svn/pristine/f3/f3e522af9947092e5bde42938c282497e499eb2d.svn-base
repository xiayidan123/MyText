...
-(void)dealloc{
    [mainImage release],mainImage = nil;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	else if (self.toolType == DAScratchPadToolTypeAirBrush) {
...
- (void) setSketch:(UIImage*)sketch
{
    ...
    imageView.contentMode = UIViewContentModeScaleAspectFit;//self.contentMode;
	imageView.image = sketch;
	imageView.backgroundColor = [UIColor clearColor];
    NSLog(@"width:%f height:%f",sketch.size.width,sketch.size.height);
    NSLog(@"ViewWidth:%f ViewHeight:%f",self.bounds.size.width,self.bounds.size.height);
	UIGraphicsBeginImageContextWithOptions(self.bounds.size, NO, 1.0);
