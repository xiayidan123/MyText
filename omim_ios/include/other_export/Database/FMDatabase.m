...
- (BOOL)open {
	...
	if(err != SQLITE_OK) {
        if(PRINT_LOG)NSLog(@"error opening!: %d", err);
		return NO;
	}
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (busyRetryTimeout && (numberOfRetries++ > busyRetryTimeout)) {
                if(PRINT_LOG)NSLog(@"%s:%d", __FUNCTION__, __LINE__);
                if(PRINT_LOG)NSLog(@"Database busy, unable to close");
                return NO;
            }
        }
        else if (SQLITE_OK != rc) {
            if(PRINT_LOG)NSLog(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (void)setCachedStatement:(FMStatement*)statement forQuery:(NSString*)query {
    //if(PRINT_LOG)NSLog(@"setting query: %@", query);
...
- (BOOL)rekey:(NSString*)key {
    ...
    if (rc != SQLITE_OK) {
        if(PRINT_LOG)NSLog(@"error on rekey: %d", rc);
        if(PRINT_LOG)NSLog(@"%@", [self lastErrorMessage]);
    }
...
- (void)compainAboutInUse {
    if(PRINT_LOG)NSLog(@"The FMDatabase %@ is currently in use.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (traceExecution && sql) {
        if(PRINT_LOG)NSLog(@"%@ executeQuery: %@", self, sql);
    }
    ...
    if (!pStmt) {
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (busyRetryTimeout && (numberOfRetries++ > busyRetryTimeout)) {
                    if(PRINT_LOG)NSLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    if(PRINT_LOG)NSLog(@"Database busy");
                ...
                if (logsErrors) {
                    if(PRINT_LOG)NSLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    if(PRINT_LOG)NSLog(@"DB Query: %@", sql);
#ifndef NS_BLOCK_ASSERTIONS
                    if (crashOnErrors) {
    ...
    if (idx != queryCount) {
        if(PRINT_LOG)NSLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        [self setInUse:NO];
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (traceExecution && sql) {
        if(PRINT_LOG)NSLog(@"%@ executeUpdate: %@", self, sql);
    }
    ...
    if (idx != queryCount) {
        if(PRINT_LOG)NSLog(@"Error: the bind count is not correct for the # of variables (%@) (executeUpdate)", sql);
        sqlite3_finalize(pStmt);
        [self setInUse:NO];
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
			...
			if (SQLITE_LOCKED == rc) {
				...
				if (rc != SQLITE_LOCKED) {
					if(PRINT_LOG)NSLog(@"Unexpected result from sqlite3_reset (%d) eu", rc);
				}
			}
            usleep(20);
            ...
            if(PRINT_LOG)NSLog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(db));
            if(PRINT_LOG)NSLog(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            if(PRINT_LOG)NSLog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(db));
            if(PRINT_LOG)NSLog(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            if(PRINT_LOG)NSLog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(db));
            if(PRINT_LOG)NSLog(@"DB Query: %@", sql);
        }
