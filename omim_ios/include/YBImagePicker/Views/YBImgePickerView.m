...
@interface YBImgePickerView ()<UINavigationControllerDelegate,YBImagePickerViewControllerDelegate,YBImagePickerView_photoViewDelegate,YBOriginalPhotoVCDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
- (void)takePhoto{
    // 判断选择的照片数量是否达到了最大选择数量
    if ([YBPhotePickerManager sharedYBPhotePickerManager].isFull){
        NSString *message_str = [NSString stringWithFormat:@"最多选取%d张照片",[YBPhotePickerManager sharedYBPhotePickerManager].max_selectedPhoto_count];
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeImage])
    {
        ...
        [self.assetLib writeImageToSavedPhotosAlbum:image.CGImage orientation:(ALAssetOrientation*)image.imageOrientation completionBlock:^(NSURL *assetURL, NSError *error) {
            [self.assetLib assetForURL:assetURL resultBlock:^(ALAsset *asset) {
                ...
                [[YBPhotePickerManager sharedYBPhotePickerManager]addPhoto:ybPhoto];
...
-(YBImagePickerViewController *)pickerViewController{
    if (_pickerViewController == nil){
        ...
        _pickerViewController.max_count = 6;
