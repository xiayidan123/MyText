...
@implementation NormalMomentCell
...
//        
       ...
       [btn_comment setTitleColor:[Colors wowtalkbiz_inactive_text_gray] forState:UIControlStateNormal];
        ...
        /*
         //favirote area
         UIImageView* favicon = [[UIImageView alloc] initWithFrame:CGRectMake(222, 20, 14, 14)];
         [favicon setImage:[UIImage imageNamed:@"timeline_favorite.png"]];
         favicon.tag = TAG_ICON_FAVORITE;
         [self.contentView addSubview:favicon];
         [favicon release];
         
         UIButton* btn_fav = [[UIButton alloc] initWithFrame:CGRectMake(210, 8, 50, 34)];
...
-(void) openDetail{
    if (![self.parent isKindOfClass:[BizMomentDetailViewController class]]) {
        ...
        [self.parent.navigationController pushViewController:bmdv animated:YES];
    }
}
...
-(void)analyzeMoment{
    ...
    MomentTextHeight = [NormalMomentCell getMomentTextHeight:_moment];
...
-(void)buildMomentArea{
    ...
    if (MomentTextHeight > 0) {
        ...
        /*自动计算UITextView的高度，有下面两行就够了，不明白为什么wowtalk要自己 计算*/
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (hasLikeRow) {
        if (indexPath.row == 0) {
            ...
            return [[dic_cellHeight objectForKey:[NSString stringWithFormat:@"%ld",(long)indexPath.row -1]] integerValue];
        }
    }
    else{
        if (self.moment.momentType == 1) {
...
+(CGFloat)heightForMoment:(Moment*)tmpmoment{
    ...
    CGFloat textheight = [NormalMomentCell getMomentTextHeight:tmpmoment];
