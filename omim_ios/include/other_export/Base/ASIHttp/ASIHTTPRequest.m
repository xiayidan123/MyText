...
- (void)updateDownloadProgress
{
    ...
    self.downloadProgress = (float)((([self totalBytesRead]+[self partialDownloadSize])*1.0) /(([self contentLength]+[self partialDownloadSize])*1.0));  // added in 12.13 , Elvis
...
- (void)updateUploadProgress
{
    ...
    self.uploadProgress = (float)((([self totalBytesSent]-[self uploadBufferSize])*1.0) / (([self postLength]-[self uploadBufferSize])*1.0)) ;  // added in 12.13 Elvis.
//	NSLog(@"%f",self.uploadProgress);
...
+ (void)registerForNetworkReachabilityNotifications
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:kReachabilityChangedNotification2 object:nil];
}


+ (void)unsubscribeFromNetworkReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:kReachabilityChangedNotification2 object:nil];
}
...
// added in 12.13  @ Elvis @synthesize
