//
//  Communicator_GetPossibleBuddyList.m
//  omimLibrary
//
//  Created by Yi Chen on 5/2/12.
...
@implementation Communicator_GetPossibleBuddyList
...
- (void)wowtalkXMLParseFinished:(NSMutableDictionary *)result
{
    ...
    if (errNo == NO_ERROR)
    {
        ...
        if (bodydict == nil || [bodydict objectForKey:XML_GET_POSSIBLE_BUDDY_LIST_KEY] == nil)
            errNo = NECCESSARY_DATA_NOT_RETURNED;
        else{
            NSMutableDictionary *buddys = [bodydict objectForKey:XML_GET_POSSIBLE_BUDDY_LIST_KEY];
            
            NSMutableArray *buddyArray = [[NSMutableArray alloc] init];
            if ([[buddys objectForKey:XML_BUDDY] isKindOfClass:[NSMutableDictionary class]])
            {
                NSMutableDictionary *buddydict = [buddys objectForKey:XML_BUDDY];
            ...
            else if ([[buddys objectForKey:XML_BUDDY] isKindOfClass:[NSMutableArray class]])
            {
                NSMutableArray *arr = [buddys objectForKey:XML_BUDDY];
            ...
            {
                [Database deleteAllPossibleBuddies];
