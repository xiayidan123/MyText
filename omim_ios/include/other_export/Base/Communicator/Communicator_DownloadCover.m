...
- (void)downloadFailed:(ASIHTTPRequest *)theRequest
{
    ...
    if(self.didFinishDelegate)
    {
        if (theRequest.error)
            ...
            [self.didFinishDelegate networkTaskDidFailWithReturningData:nil error:[NSError errorWithDomain:@"Communicator_DownloadCover" code:-10 userInfo:nil]];
    }
}
...
- (void)downloadFinished:(ASIHTTPRequest *)theRequest
{
    ...
    [MediaProcessing writeMedia:[self.image.file_id stringByAppendingPathExtension:self.image.ext] data:theRequest.responseData];
