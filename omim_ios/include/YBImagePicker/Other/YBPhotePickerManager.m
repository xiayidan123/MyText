...
-(int)max_selectedPhoto_count{
    if (_max_selectedPhoto_count == 0){
        return 9;
    }else{
        return _max_selectedPhoto_count;
    }
}

-(BOOL)isFull{
    if (self.max_selectedPhoto_count > self.selected_photo_model_array.count){
        return NO;
    }else{
        return YES;
    }
