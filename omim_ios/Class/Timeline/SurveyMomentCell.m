    ...
    if (tableView.tag == TAG_TABLE_VOTE) {
        ...
        if (hasLikeRow) {
            if (indexPath.row == 0) {
                ...
                return [[dic_cellHeight objectForKey:[NSString stringWithFormat:@"%ld",indexPath.row -1]] floatValue];
            }
        }
        return 0;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == TAG_TABLE_VOTE) {
        if ([self.moment isVotedMoment] || [self.moment isExpiredSurveyMoment]) {
            ...
            label.text = [NSString stringWithFormat:@"%zi: %@",indexPath.row+'A',option.desc];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (tableView.tag == TAG_TABLE_VOTE) {
        ...
        label_desc.minimumScaleFactor = 10;
