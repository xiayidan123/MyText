...
-(UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ((_widthOfLine + self.itemSize.width) >= _collectionViewSize.width){
        ...
        if(indexPath.row == 0)_heights = 0;
    }
    
    if (_widthOfLine == 0){
        attribute.frame = CGRectMake(_widthOfLine , _heights,self.itemSize.width , _heightOfLine);
        _widthOfLine = _widthOfLine + self.itemSize.width ;
    }else{
